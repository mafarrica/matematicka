...
def my_component(name="World"):
    return """
        ...
        return (
            ...
            <a href="https://hackbot.co/dashboard/will-main-party-education" className="relative flex items-center gap-4 bg-black p-4 transition-colors hover:bg-zinc-900">
                ...
                <span className="text-xs font-medium text-zinc-400">View Report on</span>
                <span className="text-lg font-semibold text-white">Hackbot Dashboard</span>
                </div>
                <ArrowRight className="ml-auto h-6 w-6 text-zinc-400 transition-transform group-hover:translate-x-1" />
                <div className="absolute bottom-1 left-2 text-[10px] text-zinc-700">last updated: {lastUpdated}</div>
            </a>
            </Card>
        );
        }
